...
if texture == "sand" :
    ...
    print("The texture input is not correct \n")
...
\t \t \t <static>1</static> \n \
\t \t </include> \n \n" \
%(V, 1.5 + (num - 1.0) / F + Random_quinconce, W/2 - (row - 1.0) * W + Ran / 50.0, (1.20 - row/15.0), Ran * 180.0))
            num += 1
        row += 1
        if quinconce :
...
\t \t \t </distribution>\n \
\t \t </population> \n \n"\
%(1.5 + L/2.0, -W*(N-1)/2 + W/2, L+2, W*(N-1), L*N*25))
