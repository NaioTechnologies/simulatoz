/*
 * \file  gazebo_ros_imu_direct.cpp
 *
 * \brief imu
 *
 ...
 *
 */
#include <gazebo_plugins/gazebo_ros_imu_direct.h>
...
//----------------------------------------------------------------------------------------------
    ...
    // Constructor
    GazeboRosIMUDirect::GazeboRosIMUDirect()
    {
...
// Load the controller
    void GazeboRosIMUDirect::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
        // Get the world name.
        ...
        if (!serviceName.empty())
            srv_ = node_handle_->advertiseService(serviceName, &GazeboRosIMUDirect::ServiceCallback, this);

        accelBiasService = node_handle_->advertiseService(topic_ + "/set_accel_bias", &GazeboRosIMUDirect::SetAccelBiasCallback, this);
        rateBiasService  = node_handle_->advertiseService(topic_ + "/set_rate_bias", &GazeboRosIMUDirect::SetRateBiasCallback, this);
  ...
  callback_queue_thread_ = boost::thread( boost::bind( &GazeboRosIMU::CallbackQueueThread,this ) );
        ...
        updateConnection = updateTimer.Connect(boost::bind(&GazeboRosIMUDirect::Update, this));
    }

 //----------------------------------------------------------------------------------------------

   void GazeboRosIMUDirect::Reset()
    {
        updateTimer.Reset();
...
// returns true always, imu is always calibrated in sim
    bool GazeboRosIMUDirect::ServiceCallback(std_srvs::Empty::Request &req,
...
//----------------------------------------------------------------------------------------------
    ...
    bool GazeboRosIMUDirect::SetAccelBiasCallback(hector_gazebo_plugins::SetBias::Request &req, hector_gazebo_plugins::SetBias::Response &res)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
...
//----------------------------------------------------------------------------------------------
    ...
    bool GazeboRosIMUDirect::SetRateBiasCallback(hector_gazebo_plugins::SetBias::Request &req, hector_gazebo_plugins::SetBias::Response &res)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
...
// Update the controller
    void GazeboRosIMUDirect::Update()
    {
        ...
        int now = static_cast<int64_t>( image_now_ms.count());

        int duree_entre_deux_packets = (int)now - last_gyro_packet_send_ ;

        ROS_ERROR("Duree PLUGIN %d", duree_entre_deux_packets);
...
// Register this plugin with the simulator
    GZ_REGISTER_MODEL_PLUGIN(GazeboRosIMUDirect)
