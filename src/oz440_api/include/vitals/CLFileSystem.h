...
#ifndef CLFILESYSTEM_HPP
#define CLFILESYSTEM_HPP
...
/// Visitor base interface for the EnumFiles() functionality.
class EnumVisitor
{
public:
	EnumVisitor();
	...
	virtual void on_file_found( const std::string& path, const std::string& fileName,
								bool& aborted ) = 0;

	bool is_directory() const;

	void set_directory( const bool isDirectory );

	bool was_link() const;

	void set_was_link( const bool wasLink );
	...
	: public IterateData
{
	...
	cl::filesystem::EnumVisitor* visitor;
};

bool enumerate_files( IterateData& data );

using ActionFunc = std::function< bool( IterateData& data ) >;

bool iterate_elements( const std::string& folder, const std::string& destFolder, ActionFunc fun,
					   IterateData* data, bool recursive );

void enum_files_recurs( const std::string& folder, EnumVisitor& visitor, const bool recurse,
						bool& aborted );

/// Enumerates files and folders in the given folder. Can recurse inside folders if set recurse is
/// true. Calls the visitor for every File/Dir item. Returns aborted or not. Any visitor can abort
/// the enum process.
bool enum_elements( const std::string& folderPath, EnumVisitor& visitor, bool recurse );
...
bool is_present( const std::string& thing, std::error_code& ec );

// test for whether there's something present and its a folder
bool is_folder( const std::string& thing, std::error_code& ec );

/// Test for whether there's something present and its a file. A file can be a regular file, a
/// symbolic link, a FIFO or a socket, but not a device
bool is_file( const std::string& thing, std::error_code& ec );

bool is_regular_file( const std::string& thing, std::error_code& ec );

bool is_fifo_file( const std::string& thing, std::error_code& ec );

bool is_symlink_file( const std::string& thing, std::error_code& ec );

bool is_socket_file( const std::string& thing, std::error_code& ec );

////////////////////////////////////////////////////////////////////////////////
// file functions

void create_file( const std::string& filepath, std::error_code& ec );
...
std::string create_filespec( const std::string& folder, const std::string& basename,
...
std::error_code folder_create( const std::string& folder );
...
std::vector< std::string > folder_wildcard( const std::string& folder, const std::string& wildcard,
											bool folders = true, bool files = true );
...
std::string lookup( const std::string& file, const std::string& path,
...
bool is_drive_mounted( const std::string& devPath, std::string& mountPoint );

bool mount_drive( const std::string& devPath, const std::string& dirPath, std::error_code& ec,
				  const bool readOnly = true );

std::error_code unmount_drive( const std::string& devPath );

uint64_t get_free_space( const std::string& devPath );

} // filesystem

}

#endif // CLFILESYSTEM_HPP
